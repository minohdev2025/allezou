...
const MESSAGES: Record<string, string> = {
  defi_absent: "La demande a expiré. Réessayez.",
  cle_inconnue: "Cet appareil n'est enregistré sur aucun compte.",
  cle_deja_enregistree: "Cet appareil est déjà enregistré.",
  verification_echouee: "La vérification a échoué.",
};
...
export function AjouterCleAcces({
  ...
  async function ajouter() {
    if (!disponible()) {
      setErreur(
        "Ce navigateur ne sait pas enregistrer d'appareil. Vous continuerez à recevoir un lien par courriel.",
      );
      return;
    }
    ...
    try {
      ...
      if (result.ok) {
        ...
        setEtat("repos");
        setErreur(MESSAGES[result.reason ?? ""] ?? "Cela n'a pas marché.");
      }
    } catch {
      // Un refus au moment du déverrouillage passe aussi par ici : ce n'est pas une panne.
      setEtat("repos");
      setErreur("Enregistrement annulé.");
    }
  }

  if (etat === "fait") {
    return (
      <p className="font-bold text-[color:var(--color-vert)]">
        Cet appareil est enregistré ✅ La prochaine fois, ouvrir l&apos;application suffira.
      </p>
    );
  }
  ...
  return (
    <div>
      <button onClick={ajouter} disabled={etat === "en_cours"} className={`${bouton} ${second}`}>
        {etat === "en_cours" ? "…" : "Enregistrer cet appareil 🔐"}
      </button>
      {erreur ? (
...
export function ConnexionParCleAcces({
  ...
  async function entrer() {
    if (!disponible()) {
      setErreur("Ce navigateur ne sait pas reconnaître un appareil enregistré.");
      return;
    }
  ...
  return (
    <div className="mb-5">
      <button onClick={entrer} disabled={etat === "en_cours"} className={`${bouton} ${principal}`}>
        {etat === "en_cours" ? "…" : "Entrer avec cet appareil 🔐"}
      </button>
      {erreur ? (
        <p className="mt-2 text-center text-sm text-[color:var(--color-corail)]">{erreur}</p>
      ) : null}
      <p className="mt-2 text-center text-sm text-[color:var(--color-doux)]">
        Si vous avez déjà enregistré ce téléphone.
      </p>
    </div>
  );
}
