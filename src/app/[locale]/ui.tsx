 ...
 *
 * Volontairement identique dans les cinq langues : à Genève tout le monde lit l'heure sur
 * 24 heures, et « 12:15 » n'a pas besoin de traduction. Les noms de jours et de mois, eux,
 * en ont besoin — c'est le rôle des deux fonctions du dessous, qui prennent la langue.
 */
 ...
 *
 * « Aujourd'hui » et « Demain » viennent d'`Intl.RelativeTimeFormat`, pas d'un catalogue :
 * la plateforme les connaît dans les cinq langues, et ils ne peuvent pas manquer.
 */
export function libelleJour(date: Date, locale: Locale): string {
  const bcp47 = LOCALE_BCP47[locale];
  const aujourdhui = cleDuJour(new Date());
  const demain = cleDuJour(new Date(Date.now() + 86_400_000));
  const jour = cleDuJour(date);

  const majuscule = (texte: string) => texte.charAt(0).toLocaleUpperCase(bcp47) + texte.slice(1);

  const relatif = new Intl.RelativeTimeFormat(bcp47, { numeric: "auto" });
  if (jour === aujourdhui) return majuscule(relatif.format(0, "day"));
  if (jour === demain) return majuscule(relatif.format(1, "day"));

  const libelle = new Intl.DateTimeFormat(bcp47, {
    weekday: "long",
    day: "numeric",
    month: "long",
    timeZone: "Europe/Zurich",
  ...
  return majuscule(libelle);
}

export function jourCourt(date: Date, locale: Locale): { jour: string; nombre: string; mois: string } {
  const parties = new Intl.DateTimeFormat(LOCALE_BCP47[locale], {
    weekday: "short",
    day: "numeric",
    month: "short",
    timeZone: "Europe/Zurich",
